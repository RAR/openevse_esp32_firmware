...
//   used block : [head canary 0xABBA1234][owner][alloc_size] data... [tail canary 0xBAAD5678]
...
#define TRAP_MAGIC   0x48545250u   // "HTRP"
...
static uint8_t check_block(const walker_block_info_t &b)
{
  ...
  if(b.used) {
    ...
    uint32_t alloc = w[2];
    if(alloc + 16 > b.size) return 1;
    uint32_t tail;
    memcpy(&tail, (const uint8_t *)b.ptr + 12 + alloc, 4);
...
static void block_json(JsonObject o, const BlockRec &b)
{
  ...
  o["size"] = b.size;
  o["used"] = b.used;
  o["reason"] = reasons[b.reason < 4 ? b.reason : 0];
