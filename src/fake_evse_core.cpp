...
uint8_t FakeEvseState::pilot_state() const {
  // Phase 1 of the flicker reports the "connected" pilot code instead of the
  // state code; any differing value is enough to fire the firmware's event.
  if (pilot_flicker && flicker_phase) {
    return FAKE_EVSE_CONNECTED;
  }
  return state();
}
...
std::string fake_evse_handle(FakeEvseState &st, const std::string &cmd) {
  ...
  if (c == "$GV") {
    ...
    snprintf(buf, sizeof(buf), "$OK %X %u %X %X",
             st.state(), st.session_elapsed_s, st.pilot_state(), st.vflags());
...
std::string fake_evse_tick(FakeEvseState &st, double seconds) {
  ...
  // Bench-only pilot flicker: alternate the reported pilot state and announce
  // it, leaving state/capacity/vflags alone. This is the shape of the real
  // fault - the firmware's state event triggers on pilot_state, so it fires,
  // but every field the event log writes is unchanged.
  if (st.pilot_flicker) {
    st.flicker_phase ^= 1;
    st.last_reported_state = now;
    char fbuf[32];
    snprintf(fbuf, sizeof(fbuf), "$AT %X %X %ld %X",
             now, st.pilot_state(), st.pilot_a, st.vflags());
    return rapi_frame(fbuf);
  }
