...
struct FakeEvseState {
  ...
  // Bench-only: reproduce the marginal-hardware pilot-state flicker that the
  // event log used to record as walls of identical rows. When on, the reported
  // pilot state alternates every tick while the EVSE state, capacity and vflags
  // all hold still - so the firmware's state event fires but nothing it writes
  // to the log changes. Set via POST /fakeevse {"pilot_flicker":true}.
  bool     pilot_flicker = false;
  uint8_t  flicker_phase = 0;

  uint8_t  state() const;                 // derived OpenEVSE state code
  uint8_t  pilot_state() const;           // reported pilot state (see pilot_flicker)
